...
// nome_progetto::nome_file_proto::nome_servizio+"_client"::{NomeServizio, NomeServizioServer}
use rust_grpc::helloworld::greeter_server::{Greeter, GreeterServer};
use rust_grpc::helloworld::{HelloReply, HelloRequest};
...
} // Non ha campi perché il service nel file proto non ha campi per definizione
// è come se stessimo creando una classe senza campi ma con solo metodi (vedi impl)
...
impl Greeter for GreeterImpl {
    ...
    async fn say_hello(&self, request: Request<HelloRequest>) -> Result<Response<HelloReply>, Status> {
        ...
        Ok(Response::new(HelloReply { // le struct si istanziano esattamente come in Go
...
async fn main() -> Result<(), Box<dyn std::error::Error>> { // Se va a buon fine non restituisce niente
    ...
    // Creo il servizio
    let say = GreeterImpl::default(); // istanzia la struct impostando TUTTI i valori in default!
    // aggiungo l'indirizzo al server
    println!("Server listening on {}", addr);
    Server::builder()
        .add_service(GreeterServer::new(say)) // Qua si possono aggiungere altri service se vuoi!!!
